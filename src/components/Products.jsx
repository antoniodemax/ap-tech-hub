...
const Products = () => {
  ...
  return (
    <section id="products" style={{ padding: '40px 20px', backgroundColor: '#f9f9f9', margin: '20px auto', maxWidth: '1200px', borderRadius: '10px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)' }}>
      <div style={{ backgroundColor: '#fff', borderRadius: '10px', padding: '20px' }}>
        <h2 style={{ textAlign: 'center', color: '#1e90ff' }}>Our Products</h2>
        <div className="products-container" style={{ display: 'flex', justifyContent: 'space-around', flexWrap: 'wrap' }}>
          {products.map(product => (
            <div key={product.id} className="product-card" style={{ backgroundColor: '#fff', borderRadius: '8px', padding: '20px', margin: '15px', width: '250px', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)' }}>
              <img src={product.image} alt={product.name} style={{ width: '100%', height: 'auto', marginBottom: '15px' }} />
              <h3 style={{ color: '#1e90ff' }}>{product.name}</h3>
              <p style={{ color: '#333' }}>{product.description}</p>
              <button style={{ backgroundColor: '#1e90ff', color: 'white', border: 'none', padding: '10px 15px', borderRadius: '5px' }}>Add to Cart</button>
            </div>
          ))}
        </div>
      </div>
